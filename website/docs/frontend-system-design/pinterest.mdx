...
> **Editable diagram:** [Open in Excalidraw](https://excalidraw.com/#room=c14609430f7878c9ec30,HUT8Frsvr0heNYwRqp_cmg)
 ...
 * }}
 */
function calculateMasonryLayout({ items, containerWidth, columnWidth = 240, gap = 16 }) {
...
<div class="masonry" style="height: 1200px">
  <div class="masonry-item" style="transform: translate(0px, 0px)">...</div>
  <div class="masonry-item" style="transform: translate(256px, 0px)">...</div>
</div>
```
...
export function Masonry({ items, columnWidth = 240, gap = 16 }: MasonryProps) {
...
```js
function getVisibleItems({ items, scrollTop, viewportHeight, overscan = 800 }) {
...
function ImageCard({ item }) {
  ...
  return <img src={item.src} alt={item.alt ?? ''} onError={() => setFailed(true)} />;
}
```
...
## Masonry Interview Summary Answer
